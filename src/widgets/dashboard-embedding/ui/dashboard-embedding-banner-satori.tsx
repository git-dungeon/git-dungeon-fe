...
import type { DashboardEmbeddingBannerProps } from "@/widgets/dashboard-embedding/ui/dashboard-embedding-banner";
...
} from "@/widgets/dashboard-embedding/lib/satori-presets";
...
interface DashboardEmbeddingBannerSatoriProps
  extends DashboardEmbeddingBannerProps {
...
function EquipmentCard({
  ...
  const slotLabel =
    language === "ko"
      ...
      : SLOT_LABEL_EN[item.slot];
  const rarityLabel =
    language === "ko"
      ? formatRarity(item.rarity)
      : (RARITY_LABEL_EN[item.rarity] ?? item.rarity);
  const effectLabel = item.effect
    ? language === "ko"
      ? formatInventoryEffect(item.effect)
      : (INVENTORY_EFFECT_LABEL_EN[item.effect.type] ?? item.effect.type)
    : null;
...
function EmptyEquipmentCard({
  ...
  const slotLabel =
    language === "ko" ? getInventorySlotLabel(slot) : SLOT_LABEL_EN[slot];
...
function formatLocaleNumber(value: number, language: EmbedPreviewLanguage) {
