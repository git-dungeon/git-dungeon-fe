...
function isValidFilterType(type: string | null): type is LogsFilterType {
  return Boolean(
    type && (LOGS_FILTER_TYPES as readonly string[]).includes(type)
  );
}

function filterLogs(logs: LogEntry[], filterType?: LogsFilterType): LogEntry[] {
  if (!filterType) {
    return logs;
  }
...
export const logsHandlers = [
  http.get(DASHBOARD_ENDPOINTS.logs, ({ request }) => {
    ...
    const sourceLogs = filterLogs(mockLogs, filterType);
