...
const ACTION_IMAGE_MAP: Partial<Record<LogAction, string>> = {
  BATTLE: battleImage,
  DEATH: deadImage,
...
export function resolveActionThumbnail(action: LogAction) {
...
function resolveGoldBadge(entry: LogEntry): LogThumbnailBadge | undefined {
...
export function buildLogThumbnails(
  ...
  const pushRewardItems = (items?: LogRewardItem[]) => {
    if (!items?.length) {
      return;
    }
  ...
  const pushInventoryItems = (
    items: LogInventoryDeltaItem[] | undefined,
    badge: LogThumbnailBadge,
    idPrefix = "item"
  ) => {
    if (!items?.length) {
